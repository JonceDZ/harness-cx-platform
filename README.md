...
3. **Describe the agent you want** in the orchestrator chat (e.g. “Build an agent that looks up orders by ID”). The harness will intake, plan, implement, test, review, and document.
...
Agent code is written to `agent/` (same level as `.cursor`), not mixed with harness config:
...
├── .gitignore
└── agent/                # the Weni agent — safe to push to CX Platform
...
python .cursor/scripts/init_run.py "feature"     # start a new run
python .cursor/scripts/validate_schema.py      # validate agent/ after implement
python .cursor/scripts/run_tool_tests.py       # weni run ... -v per tool → agent/test-results.md
python .cursor/scripts/run_eval.py --latest    # weni eval run → run artifact 03-tests.md
