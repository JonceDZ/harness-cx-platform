...
def project_root() -> Path:
    """Return the project root (two levels above the .claude/scripts folder)."""
    return Path(__file__).resolve().parents[2]


def runs_dir() -> Path:
    """Return the directory that holds all runs."""
    return project_root() / ".claude" / "runs"
...
def agents_root() -> Path:
    ...
    level as .claude, with its own `agent_definition.yaml`, `tools/`, and eval.
